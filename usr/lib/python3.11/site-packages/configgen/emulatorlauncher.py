...
def start_rom(args: argparse.Namespace, maxnbplayers: int, rom: str, romConfiguration: str) -> int:
    ...
    # DMT - Swap USB encoders for 8bit do (or wheel) controller(s)
    if swap_controllers(args.system, romFile):
        if len(player_controllers) > 2:
            player_controllers[1], player_controllers[3] = player_controllers[3], player_controllers[1]
            if len(player_controllers) == 4:
                player_controllers[2], player_controllers[4] = player_controllers[4], player_controllers[2]
                
            eslog.debug(f"Controllers swapped.  Number of controllers: {len(player_controllers)} and rom {romFile} configured for controller swap")
