...
# DMT - lookup whether the <system/rom file name> exists for player 1 <-> 3 and 2 <-> 4 controller swapping
def swap_controllers(system: str, romFileName: str) -> bool:
    with open("/userdata/system/configs/controller/controller_swap.cfg", 'r') as file:
        lines = file.readlines()

        for line in lines:
            if line.strip() == system + "/" + romFileName:
                return True
...
def start_rom(args: argparse.Namespace, maxnbplayers: int, rom: str, romConfiguration: str) -> int:
    ...
    romFile = os.path.basename(args.rom)
    ...
    # Controller priority order checks.  If a priority order has been provided and we have a list of controllers (aliases) to apply, then we will swap controls to align to that order, up to the number of players (if provided)
    reordered_player_controllers = {}
    try:
        if controllerOrderListString := system.getOptString('controller.order'):
            controllerOrderList = controllerOrderListString.split(',')

            controllerListString = system.getOptString('controller.aliases')
            controllerList = controllerListString.split(',')
            
            # Get the real name of all aliased devices, so we can match the list provided by ES below 
            controllerRealNameList = [];
            for controllerAlias in controllerList:
                if currControllerRealName := system.getOptString('controller.alias.' + controllerAlias):
                    controllerRealNameList.append(currControllerRealName)
                  
            # Get number of players to configure for available controllers, use 4 players as default
            numPlayers = 4
            if numPlayer := system.getOptString('controller.players'):
                if numPlayer.isnumeric():
                    numPlayers = int(numPlayer)
            
            # Only continue if a defined set of controller aliases has been provided and real names found
            if len(controllerList) > 0 and len(controllerRealNameList) > 0:
                # Loop through number of players to assign, find the first ES controller meeting the
                # preference (order) for the players
                for i in range(numPlayers):
                    player_assigned = False
                    for j in range(len(controllerOrderList)):
                        # Loop through each preferred controller type in order
                        k = controllerList.index(controllerOrderList[j])
                        for playercontroller, pad in player_controllers.items():
                            # Check if we have remaining ES controllers to assign, if not already assigned then give to the player (and update the internal pad index with the new player assignment).  We mark the pad as assigned so we don't give it to another player
                            if controllerRealNameList[k] == pad.real_name.strip() and not pad.assigned:
                                reordered_player_controllers[i + 1] = pad
                                pad.assigned = True
                                pad.player_number = i + 1
                                player_assigned = True
                                
                                break

                        if player_assigned:
                            # No need to traverse the rest of the ES controllers or preferred order once we've assigned a controller
                            break
    except Exception as e:
        # Incase the config is broken in batocera.conf, wrie the error and we'll just user the default player_controllers (order)
        eslog.debug(f'Failed to re-order controllers.  Default controller order will be used.  Exception: {e}') 
        pass
        
    if reordered_player_controllers:
       player_controllers = reordered_player_controllers
