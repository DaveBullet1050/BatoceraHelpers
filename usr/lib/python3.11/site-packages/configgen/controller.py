...
def _key_to_sdl_game_controller_config(keyname: str, input: Input, /) -> str | None:
    ...
    """
#    eslog.info("keyname:" + keyname)
#    eslog.info("input.type:" + input.type)
#    eslog.info("input.id:" + input.id)
#    eslog.info("input.value:" + input.value)
...
class Controller:
    ...
    system: Emulator
    skip_dpad: bool = False
    add_leftx: bool = False
    ...
    def generate_sdl_game_db_line(self, sdl_mapping: Mapping[str, str] = _DEFAULT_SDL_MAPPING, /) -> str:
        ...
        def add_mapping(input: Input) -> None:
            ...
            if key_name is None or (self.skip_dpad and key_name in ('dpup', 'dpdown', 'dpleft', 'dpright')):
                return
        ...
        # DMT - another hack. "leftx:a0,lefty:a1" is not being added / found for Dragonwise
        # encoders and this is needed to enable joystick for broforce, so add in if missing
        if self.add_leftx:
            if "leftx:a0,lefty:a1" not in config:
                config.append("leftx:a0,lefty:a1")
...
def generate_sdl_game_controller_config_system(controllers: ControllerMapping, system: Emulator, /) -> str:
    for controller in controllers.values():
        # DMT - check if the controller exists with either skip_dpad generation or adding the
        # leftx etc... parameters to the generated SDL_GAMECONTROLLERCONFIG for windows games
        # the setting in batocera.conf is prefixed with the first part of the controller name
        # e.g. for a "DragonRise Inc.   Generic   USB  Joystick  " controller, this looks for
        # "dragonwise_skip_dpad" and "dragonwise_add_leftx" in the batocera.conf for either
        # the system or the specific game
        controller.system = system
        controller_name, _, _ = controller.real_name.partition(" ")
        controller_name = controller_name.lower()
        controller.skip_dpad = system.getOptBoolean(controller_name + "_skip_dpad")
        controller.add_leftx = system.getOptBoolean(controller_name + "_add_leftx")
    return generate_sdl_game_controller_config(controllers)

def write_sdl_controller_db(
    controllers: ControllerMapping, outputFile: str | Path = "/tmp/gamecontrollerdb.txt", /,) -> Path:
