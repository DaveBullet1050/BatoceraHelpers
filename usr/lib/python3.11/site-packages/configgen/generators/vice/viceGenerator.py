...
class ViceGenerator(Generator):
    ...
    def generate(self, system, rom, playersControllers, metadata, guns, wheels, gameResolution):
        ...
        # DMT - if (NTSC) found in filename, then force the correct model
        if "(NTSC)" in rom:
            commandArray.append("-model")
            commandArray.append("vic20ntsc")
        
        # DMT - Some VIC-20 games need to have this commandline argument to attach the game as a generic
        # Cartridge (e.g. Road Race.prg).  This hack means if you include "[CG]" somewhere in the ROM
        # filename, the rom will be attached as a generic cartridge
        if "[CG]" in rom:
            commandArray.append("-cartgeneric")
        #if ".crt" in rom_extension:
        #    commandArray.append("-cartcrt")
