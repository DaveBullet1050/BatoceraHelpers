...
class evmapy(AbstractContextManager[None, None]):
    ...
    def __prepare(self) -> bool:
        # DMT - the script below doesn't build a suitable AXIS_Y and AXIS_X for the joystick
        # This stops vpinball joystick down working.  Instead just use a raw evmapy json event_type
        # file if present for both player controllers
        #print(str(self.controllers))
        source_raw = "/userdata/system/configs/evmapy/{}.json".format(self.system)
        if os.path.exists(source_raw):
            ...
            for playercontroller, pad in sorted(self.controllers.items()):
                target_file = "/var/run/evmapy/{}.json" .format (os.path.basename(pad.device_path))
                os.popen("cp {} {}".format(source_raw, target_file))
            return True
        ...
        if keysfile is not None:
            ...
            for playercontroller, pad in sorted(self.controllers.items()):
                if "actions_player"+str(nplayer) in padActionConfig:
                    ...
                    # DMT - Hack.  use evmapy for all events, not passing any through to VPinball
                    #padConfig["grab"] = False                    
                    padConfig["grab"] = (self.system == "vpinball")
